...
def scrape_tours(url):  # función que sirve para obtener los links en vivo de cada torneo
    ...
    df = df.drop(range(df.index[df['Torneo'] == "2024/regional-charlotte/masters"].tolist()[0] + 1, len(df)),
                 errors='ignore')
...
fact_player_tour.insert(0, 'ID', range(1001, 1001 + len(fact_player_tour)))
